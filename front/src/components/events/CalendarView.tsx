...
const CalendarView = ({ events, refreshEvents = () => {} }: any) => {
  ...
  const joinEvent = async () => {
    await api
      .joinEvent({ eventId: clickedEvent.id, userId: user?.id })
      .then((response) => {
        toast.success("Vous avez rejoint l'événement avec succès !");
        refreshEvents();
        setClickedEvent({
          ...clickedEvent,
          participants: [...clickedEvent.participants, user],
        });
      })
      .catch((error) => {
    ...
    return (
      <div style={{ display: "flex", justifyContent: "center", marginTop: 15 }}>
        <Card sx={{ minWidth: 275, width: 450 }}>
          <CardContent>
            <div>
              ...
              <Typography sx={{ mb: 1.5 }} color="text.primary">
                Début :{" "}
                {new Date(clickedEvent.startDate).toLocaleString("fr-FR", {
                  timeZone: "UTC",
                })}
              </Typography>
              <Typography sx={{ mb: 1.5 }} color="text.primary">
                Fin :{" "}
                {new Date(clickedEvent.endDate).toLocaleString("fr-FR", {
                  timeZone: "UTC",
                })}
              </Typography>
            </div>
          ...
          <CardActions>
            {user?.role === ROLES.user && doesParticipate?.length == 0 && (
    ...
    <>
      <FullCalendar
        ...
        eventClick={(info) => {
          ...
          doesParticipate = clickedEvent?.participants?.filter(
            (participant: any) => {
              return participant.id === user?.id;
            }
          );
